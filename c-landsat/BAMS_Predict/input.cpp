...
bool PredictBurnedArea::calcBands(Input_t *ds_input) {
	float path = (float)ds_input->meta.path;
	float row = (float)ds_input->meta.row;
	Date_t acqdate = ds_input->meta.acq_date;
	float mth = (float)acqdate.month;

	///for (int i = 0; i < arrSz; i++) {
	for (int i = 0; i < ds_input->size.s; i++) {
		predMat.at<float>(i,6) = path;
		predMat.at<float>(i,7) = row;
		predMat.at<float>(i,8) = mth;
		predMat.at<float>(i,9) = (predMat.at<float>(i,2) - predMat.at<float>(i,3))/(predMat.at<float>(i,3) + predMat.at<float>(i,2));; //NDVI - using bands 4 and 3
		predMat.at<float>(i,10) = (predMat.at<float>(i,3) - predMat.at<float>(i,5))/(predMat.at<float>(i,3) + predMat.at<float>(i,5)); //NBR - using bands 4 and 7
		predMat.at<float>(i,11) =  (predMat.at<float>(i,3) - predMat.at<float>(i,4))/(predMat.at<float>(i,3) + predMat.at<float>(i,4)); //NDMI - using bands 4 and 5

		//predMat.at<float>(i,10) = (predMat.at<float>(i,2) - predMat.at<float>(i,3))/(predMat.at<float>(i,3) + predMat.at<float>(i,2));; //NDVI - using bands 4 and 3
		//predMat.at<float>(i,11) = (predMat.at<float>(i,3) - predMat.at<float>(i,5))/(predMat.at<float>(i,3) + predMat.at<float>(i,5)); //NBR - using bands 4 and 7
		//predMat.at<float>(i,12) =  (predMat.at<float>(i,3) - predMat.at<float>(i,4))/(predMat.at<float>(i,3) + predMat.at<float>(i,4)); //NDMI - using bands 4 and 5
	}
